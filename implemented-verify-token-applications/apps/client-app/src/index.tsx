...
app.get('/callback', async (c) => {
  ...
  const tokenJson = await tokenRes.json() as { access_token: string, scope: string, expires_in: number }

  /**
   * トークンを画面に表示する代わりに、KVへ保存する
   * セッションIDを生成してKVのキーとし、CookieにはセッションIDだけを渡す
   */
  const sessionId = crypto.randomUUID()
  await c.env.MY_KV_NAMESPACE.put(sessionId, JSON.stringify({
    accessToken: tokenJson.access_token,
    scope: tokenJson.scope,
  }), { expirationTtl: tokenJson.expires_in }) // トークンの有効期限に合わせて自動削除
  setCookie(c, 'client_session', sessionId, {
    httpOnly: true,
    secure: true,
    sameSite: 'Lax',
    path: '/',
  ...
  return c.html(
    <div>
      ...
      <a href="/resource">リソースにアクセスする</a>
    </div>
  )
})

// リソースサーバーへのアクセスを中継するプロキシ
app.get('/resource', async (c) => {
  // CookieのセッションIDをキーに、KVからアクセストークンを取得する
  const sessionId = getCookie(c, 'client_session')
  if (!sessionId) {
    return c.html(<div>セッションが存在しません。フローを最初からやり直してください</div>, 401)
  }
  const saved = await c.env.MY_KV_NAMESPACE.get(sessionId)
  if (!saved) {
    return c.html(<div>トークンが見つかりません。フローを最初からやり直してください</div>, 401)
  }
  const { accessToken } = JSON.parse(saved) as { accessToken: string }

  // Bearerトークンを付与してリソースサーバーにリクエストする
  const res = await fetch(`${c.env.RESOURCE_SERVER_URI}/api/profile`, {
    headers: { Authorization: `Bearer ${accessToken}` }
  })
  if (!res.ok) {
    return c.html(<div>リソースの取得に失敗しました（ステータス：{res.status}）</div>, 500)
  }
